...
and g' env = function (* 各命令の即値最適化 (caml2html: simm13_gprime) *)
  | Add(x, V(y), id) when M.mem y env -> Format.eprintf "add %s %s\n" x y; Add(x, C(M.find y env), id)
  | Add(x, V(y), id) when M.mem x env -> Format.eprintf "add %s %s\n" x y; Add(y, C(M.find x env), id)
  | Sub(x, y, id) when M.mem y env -> Format.eprintf "sub %s %s\n" x y; Add(x, C(-(M.find y env)), id)
  | Mul(x, y, id) when M.mem x env && M.find x env = 1 -> Format.eprintf "mul %s %s\n" x y; Mov(y, id)
  | Mul(x, y, id) when M.mem x env && M.find x env = -1 -> Format.eprintf "mul %s %s\n" x y; Neg(y, id)
  | Mul(x, y, id) when M.mem y env && M.find y env = 1 -> Format.eprintf "mul %s %s\n" x y; Mov(x, id)
  | Mul(x, y, id) when M.mem y env && M.find y env = -1 -> Format.eprintf "mul %s %s\n" x y; Neg(x, id)
  | Div(x, y, id) when M.mem y env && M.find y env = 1 -> Format.eprintf "div %s %s\n" x y; Mov(x, id)
  | Div(x, y, id) when M.mem y env && M.find y env = -1 -> Format.eprintf "div %s %s\n" x y; Neg(x, id)
